...
def generate_individual_preferences(occupation: str, time_pref: str, stats: Dict) -> Dict:
    ...
    # Note: Meal times are now fixed in the backend (07:30, 12:30, 19:00)
    # Only durations are used from preferences

    work_preferences = assign_work_preferences(occupation, wake_up_time, morning_routine_duration)
    preferences = {
...
def assign_work_preferences(occupation: str, wake_up_time: datetime.time, morning_routine_duration: int) -> Dict:
    """
    ...
    earliest_work_start_time = wake_up_datetime + timedelta(minutes=morning_routine_duration + breakfast_duration + 15)
    ...
    if occupation in ["Office Worker", "Remote Worker", "Freelancer"]:
        ...
        preferred_work_duration = max(240, int(np.random.normal(480, 60)))
        ...
        preferred_work_start_time_dt = generate_random_time_within_range(earliest_work_start_time, latest_work_start_time)
        preferred_work_start_time_dt = datetime.combine(datetime.today(), round_time_to_nearest_minutes(preferred_work_start_time_dt.time(), base=15))
        preferred_work_duration = max(180, int(np.random.normal(360, 60)))
        preferred_work_start_time = preferred_work_start_time_dt.time().strftime(fmt)
    else:
