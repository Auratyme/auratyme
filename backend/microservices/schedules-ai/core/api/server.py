...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if disable_db:
        ...
        from api.db import init_db_pool, create_tables
        try:
            await init_db_pool()
            await create_tables()
            db_initialized = True
    ...
    if not disable_db and db_initialized:
        try:
...
@app.middleware("http")
async def force_simplified_format_for_ai_test(request: Request, call_next):
    """
    Middleware to ensure that responses for /ai-test/schedules/generate are in the simplified format.
    This is a workaround for the nginx proxy configuration.
    """
    is_schedule_generate = request.url.path == "/v1/schedule/generate"
    referer = request.headers.get("referer", "")
    original_url = request.headers.get("x-original-uri", "")
    forwarded_uri = request.headers.get("x-forwarded-uri", "")

    is_ai_test_endpoint = ("/ai-test/schedules/generate" in referer or
                          "/ai-test/schedules/generate" == original_url or
                          "/ai-test/schedules/generate" == forwarded_uri)

    if is_schedule_generate and is_ai_test_endpoint:
        logger.info(f"Detected request from /ai-test/schedules/generate to /v1/schedule/generate")
        scope = request.scope.copy()
        headers = list(scope["headers"])
        headers.append((b"x-force-simplified-format", b"true"))
...
logger.info("API routers included (Health, V1 Users, V1 Schedule, V1 Feedback, V1 APIdog).")
