...
from src.services.llm import LLMEngine, ModelConfig, ModelProvider
...
def get_chronotype_analyzer() -> ChronotypeAnalyzer:
    ...
    Educational Note:
        ChronotypeAnalyzer uses research-based MEQ ranges and prime windows
        from config.py, no runtime configuration needed.
    """
...
def get_wearable_service(
    adapter: DeviceDataAdapter = Depends(get_device_adapter),
    calculator: SleepCalculator = Depends(get_sleep_calculator),
) -> WearableService:
...
def get_scheduler(
    sleep_calc: SleepCalculator = Depends(get_sleep_calculator),
    chrono_analyzer: ChronotypeAnalyzer = Depends(get_chronotype_analyzer),
    prioritizer: TaskPrioritizer = Depends(get_task_prioritizer),
    solver: ConstraintSchedulerSolver = Depends(get_constraint_solver),
    llm: Optional[LLMEngine] = Depends(get_llm_engine),
) -> Scheduler:
    """
